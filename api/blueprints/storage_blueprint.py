...
def create_storage_blueprint(
    storage_service: StorageService, auth_required
) -> Blueprint:
    ...
    def signup():
        ...
        if not WALLET_RE.match(wallet_address):
            raise ValidationError("Invalid wallet address.")

        storage_service.register(
            username, password, wallet_address, available_space
        )
    ...
    def get_storage_info(username: str):
        availability, contracts_info = storage_service.get_storage_info(
            username
        )
        return (
            jsonify(
                {"availability": availability, "contracts": contracts_info}
            ),
            200,
        )
    ...
    def update_connection(username: str):
        ...
        if not isinstance(ip_address, str) or not IP_RE.match(ip_address):
